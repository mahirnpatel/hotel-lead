...
def search_cities(query: str):
    if not query: return []
    return [c for c in US_CITIES if query.lower() in c.lower()]


def search_states(query: str):
    if not query: return []
    return [s for s in US_STATES if query.lower() in s.lower()]
...
[data-testid="stDateInput"] input {
    ...
    font-size: 0.9rem !important; padding: 0.6rem 0.9rem !important; height: 42px !important;
}
[data-testid="stTextInput"] input:focus,
[data-testid="stDateInput"] input:focus { border-color: #3B82F6 !important; box-shadow: 0 0 0 2px rgba(59,130,246,0.15) !important; }
[data-testid="stTextInput"] label,
[data-testid="stDateInput"] label { color: #94A3B8 !important; font-size: 0.72rem !important; font-family: 'DM Mono', monospace !important; letter-spacing: 0.08em !important; text-transform: uppercase !important; margin-bottom: 0.3rem !important; }

/* Searchbox styling to match date inputs */
[data-testid="stSearchbox"] > div > div > input {
    background: #111927 !important; border: 1px solid #1E2D45 !important; border-radius: 8px !important;
    color: #E2E8F0 !important; font-family: 'DM Sans', sans-serif !important;
    font-size: 0.9rem !important; height: 42px !important;
}
[data-testid="stSearchbox"] > div > div > input:focus { border-color: #3B82F6 !important; box-shadow: 0 0 0 2px rgba(59,130,246,0.15) !important; }
...
[data-testid="stExpander"] { background: #0A1628 !important; border: 1px solid #1E2D45 !important; border-radius: 8px !important; margin-bottom: 0.5rem !important; }
[data-testid="stExpander"] summary { padding: 0.75rem 1rem !important; font-family: 'DM Sans', sans-serif !important; font-size: 0.9rem !important; color: #E2E8F0 !important; }
[data-testid="stExpander"]:hover { border-color: #2D4A6B !important; }

/* Enrich button — green accent */
.enrich-btn button {
    background: rgba(16,185,129,0.12) !important; border: 1px solid rgba(16,185,129,0.4) !important; color: #34D399 !important;
    font-size: 0.75rem !important; padding: 0.3rem 0.85rem !important; width: auto !important;
    font-family: 'DM Mono', monospace !important; letter-spacing: 0.06em !important;
    border-radius: 6px !important; font-weight: 600 !important; height: auto !important;
}
.enrich-btn button:hover { background: rgba(16,185,129,0.2) !important; border-color: #34D399 !important; box-shadow: none !important; transform: none !important; }

/* Logout / Start Over ghost buttons */
.logout-btn button {
...
with col_btns:
    ...
    has_report = st.session_state.get("agent1_report") is not None
    if has_report:
        b1, b2 = st.columns(2)
        with b1:
...
with col1:
    st.markdown('<div style="font-family:DM Mono,monospace;font-size:0.72rem;color:#94A3B8;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.3rem">City</div>', unsafe_allow_html=True)
    city = st_searchbox(search_cities, placeholder="e.g. Dallas", key="city_search")
with col2:
    st.markdown('<div style="font-family:DM Mono,monospace;font-size:0.72rem;color:#94A3B8;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.3rem">State</div>', unsafe_allow_html=True)
...
if "agent1_report" in st.session_state:
    ...
    st.markdown('<div class="infobanner">Expand any event to view details. Click <strong>Enrich →</strong> to run deep research on that event and proceed to outreach.</div>', unsafe_allow_html=True)

    for ev in events:
        with st.expander(ev.title, expanded=False):

            # ── Summary row ────────────────────────────────────────────────────
            sum_cols = st.columns([2.5, 1.5, 1.2, 1.5, 1.8, 1.5])
            with sum_cols[0]:
                st.markdown(f'<div style="font-size:0.875rem;color:#E2E8F0;font-weight:600;padding:0.4rem 0">{ev.title}</div>', unsafe_allow_html=True)
            with sum_cols[1]:
                st.markdown(f'<div style="font-size:0.83rem;color:#94A3B8;font-family:DM Mono,monospace;padding:0.4rem 0">{ev.start or "—"}</div>', unsafe_allow_html=True)
            with sum_cols[2]:
                st.markdown(f'<div style="padding:0.3rem 0">{score_badge(ev.relevance_score)}</div>', unsafe_allow_html=True)
            with sum_cols[3]:
                st.markdown(f'<div style="padding:0.3rem 0"><span class="category-tag">{ev.category or "—"}</span></div>', unsafe_allow_html=True)
            with sum_cols[4]:
                st.markdown(f'<div style="font-size:0.83rem;color:#94A3B8;font-family:DM Mono,monospace;padding:0.4rem 0">{fmt_attendance(ev.phq_attendance)}</div>', unsafe_allow_html=True)
            with sum_cols[5]:
                st.markdown('<div class="enrich-btn">', unsafe_allow_html=True)
                if st.button("Enrich →", key=f"enrich_{ev.id}"):
                    st.session_state["selected_event_ids"] = {ev.id}
